...
export default function PromotionManager({ lang, promotions, productsList, currentUser }: PromotionManagerProps) {
    ...
    try {
      if (f.type.startsWith('video/')) {
        if (f.size > 650 * 1024) {
          alert(
            lang === 'fr'
              ? 'Fichier vidéo trop volumineux pour Firestore (max 650 Ko). Pour les vidéos plus longues, collez un lien URL direct (https://...).'
              : 'حجم الفيديو كبير جداً للتخزين المباشر (أقصى حد 650 كيلوبايت). للفيديوهات الكبيرة يرجى وضع رابط فيديو مباشر (https://...).',
            'error'
          );
          return;
        }
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    if (imageUrl && imageUrl.startsWith('data:') && imageUrl.length > 950000) {
      alert(
        lang === 'fr'
                ...
                <div className="flex items-center gap-3">
                  {promo.imageUrl ? (
                    isVid ? (
                      ...
                      <img src={promo.imageUrl} alt="" width={48} height={48} loading="lazy" decoding="async" style={{ aspectRatio: '1 / 1' }} className="w-12 h-12 object-cover rounded-lg border border-slate-200 shrink-0" referrerPolicy="no-referrer" />
                    )
                  ) : (
              ...
              {imageUrl && (
                <div className="relative border border-slate-100 rounded-xl overflow-hidden shadow-sm bg-white mt-2">
                  {isVideoSource(imageUrl) ? (
                    ...
                    <img src={imageUrl} alt="preview" width={400} height={112} loading="lazy" decoding="async" style={{ aspectRatio: '400 / 112' }} className="w-full h-28 object-cover" referrerPolicy="no-referrer" />
                  )}
                  <button
                    type="button"
                    onClick={() => setImageUrl('')}
